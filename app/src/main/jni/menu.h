...
static void CheckAppUpdate() {
    thread([]() {
        // Using our own server now instead of Firebase/Firestore.
        // Expected JSON response shape:
        // { "force_update": bool, "required_version": int, "download_url": string }
        string response = HttpGet(UPDATE_CHECK_URL);
        if (!response.empty()) {
            try {
                auto respJson = json::parse(response);

                bool forceUpdateVal   = respJson.value("force_update", false);
                int  requiredVersion  = respJson.value("required_version", 1);
                g_UpdateURL           = respJson.value("download_url", "");
...
static bool SidebarButton(const char* label, const char* iconStr, bool selected, float width) {
    ...
    // Draw icon text centered: Purple when selected/unselected, highlighting when active
    if (iconStr && g_IconFont) {
        ...
        ImU32 tint = selected ? IM_COL32(190, 40, 230, 255) : IM_COL32(110, 30, 140, 180);
        
        if (selected) {
            // Text bloom shadow
            dl->AddText(iconPos - ImVec2(1.0f, 0.0f), IM_COL32(160, 40, 210, 80), iconStr);
            dl->AddText(iconPos + ImVec2(1.0f, 0.0f), IM_COL32(160, 40, 210, 80), iconStr);
            dl->AddText(iconPos - ImVec2(0.0f, 1.0f), IM_COL32(160, 40, 210, 80), iconStr);
            dl->AddText(iconPos + ImVec2(0.0f, 1.0f), IM_COL32(160, 40, 210, 80), iconStr);
        }
...
static bool ToggleSwitch(const char* label, bool* v) {
    ...
    ImVec4 onColor = ImVec4(0.63f, 0.16f, 0.82f, 1.0f);
...
static bool CustomSliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = "%.2f") {
    ...
    dl->AddText(ImVec2(bb.Max.x - totalSideOffset - valSize.x, bb.Min.y + 5.0f), IM_COL32(160, 40, 210, 255), value_buf);
    ...
    // Slider Fill
    if (grabPos > 0.0f) {
        dl->AddRectFilled(barPos, ImVec2(barPos.x + grabPos, barEnd.y), IM_COL32(160, 40, 210, 255), barRadius);
    }
...
static bool SegmentedControl(const char* label, int* current, const std::vector<std::string>& options, float controlHeight = 110.0f) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (*current == i) {
            dl->AddRectFilled(itemMin, itemMax, IM_COL32(160, 40, 210, 255), 12.0f);
        } else if (hovered) {
...
static bool GlassSegmentedControl(const char* label, int* current, const std::vector<std::string>& options, float controlHeight = 100.0f) {
    ...
    dl->AddRectFilled(selMin, selMax, IM_COL32(160, 40, 210, 180), 10.0f);
...
INLINE void DrawExpired(ImGuiIO& io) {
    ...
    if (Begin(O("##ExpiredWin"), nullptr,
        ...
        TextColored(ImVec4(0.85f, 0.85f, 0.90f, 1.0f), "%s",
            O("Key Expired. Buy on our Telegram @ue_euu"));
        PopTextWrapPos();
...
INLINE void DrawAutoQueue() {
    if (((g_AuthToken ^ 0xDEADBEEFCAFEBABE) == g_ExpiryTime && g_ExpiryTime > 0) || (!g_Token.empty() && !g_Auth.empty() && g_Token == g_Auth) || DEBUG_BYPASS_LOGIN) {
        ...
        PushStyleColor(ImGuiCol_Border, ImVec4(0.63f, 0.16f, 0.82f, 0.8f)); // Dark Purple Border
        ...
        if (Begin(O("##AutoQueueCD"), nullptr,
            ...
            TextColored(ImVec4(0.63f, 0.16f, 0.82f, 1.0f), "%s", count_str.c_str());
            SetWindowFontScale(1.0f);
...
static void DrawCalculating(ImGuiIO& io) {
    ...
    PushStyleColor(ImGuiCol_WindowBg, IM_COL32(21, 21, 21, 255));
    PushStyleColor(ImGuiCol_Border, IM_COL32(160, 40, 210, 255));
    PushStyleVar(ImGuiStyleVar_WindowRounding, 18.0f);
...
static void DrawContentArea(float winW, float winH) {
    ...
    float headerH = 95.0f;
    {
        ...
        // Primary Text: "AKRO GOXI" in Arial Black
        ...
        auto DrawTimeBox = [&](int value, float& curX, bool drawColonBefore) {
            ...
            // Box Background
            dl2->AddRectFilled(boxP, boxP + ImVec2(boxW, boxH), IM_COL32(160, 40, 210, 255), 10.0f);
    ...
    PushStyleColor(ImGuiCol_ScrollbarGrab, IM_COL32(160, 40, 210, 160)); // Purple grab semi-transparent
    PushStyleColor(ImGuiCol_ScrollbarGrabHovered, IM_COL32(190, 80, 230, 220));
    PushStyleColor(ImGuiCol_ScrollbarGrabActive, IM_COL32(160, 40, 210, 255));
    ...
    switch (g_menu.currentTab) {
        case 0: {
            ...
            Dummy(ImVec2(0, 15));
            {
                ...
                dl->AddLine(ImVec2(wp.x + 50.0f, cp.y), ImVec2(wp.x + contentW - 50.0f, cp.y), IM_COL32(160, 40, 210, 100), 1.0f);
            }
            Dummy(ImVec2(0, 15));
            SetCursorPosX(45.0f);
            if (g_ArialBlackFont) PushFont(g_ArialBlackFont);
            ...
            Dummy(ImVec2(0, 20));
            {
                ...
                ImU32 shadowCol = IM_COL32(160, 40, 210, (int)(40 * anim));
                float rounding = 25.0f; // Increased for a smoother look
                for(int i=1; i<=8; i++) 
                    dl2->AddRect(p - ImVec2(i,i), p + size + ImVec2(i,i), IM_COL32(150, 30, 190, (int)((20-i*2)*anim)), rounding + i, 0, 1.0f);

                // 2. Layer: Main Body
                ImU32 colBody = hovered ? IM_COL32(190, 60, 230, 255) : IM_COL32(160, 40, 210, 255);
                if (held) colBody = IM_COL32(120, 25, 155, 255);
            ...
            if (persistent_int[O("iAutoQueue_Mode")] == 2) {
                ...
                for (int i = 0; i < 17; i++) {
                    ...
                    if (isSel) {
                        ...
                        // Premium Glow Bloom
                        for(int j=1; j<=5; j++) 
                            dl2->AddRect(p - ImVec2(j,j), p + size + ImVec2(j,j), IM_COL32(150, 30, 190, 12 - j*2), rounding + j, 0, 1.0f);
...
static void DrawToasts(ImDrawList* draw) {
    ...
    for (int i = 0; i < (int)g_Toasts.size(); i++) {
        ...
        // Draw static dim purple border as base
        draw->AddRect(p_min, p_max, IM_COL32(70, 15, 90, (int)(t.alpha * 255)), 6.0f, 0, 2.0f);
        ...
        draw->AddRect(p_min, p_max, IM_COL32(190, 60, 230, (int)(t.alpha * 255)), 6.0f, 0, 2.0f); // Bright Purple Highlight
        draw->PopClipRect();
...
static void DrawToggleButton(bool cancelMode) {
    ...
    if (Begin(O("##ToggleBtn"), nullptr,
        ...
        // Stylish Glow (Static)
        dl->AddCircle(center, r + 2.0f, IM_COL32(160, 40, 210, 100), 0, 3.0f);
        
        // Circular Border for Toggle - Static thickness, no wobble
        dl->AddCircle(center, r, IM_COL32(160, 40, 210, 255), 0, 4.5f);
        ...
        dl->AddCircle(center, currentInnerR, IM_COL32(190, 90, 230, 150), 0, 2.0f);
...
static void DrawFloatingButton(ImGuiIO& io) {
    ...
    if (Begin(O("##FloatBtn"), nullptr,
        ...
        if (AutoPlay::bAutoPlaying) {
            ...
            dl->AddCircle(center, buttonRadius + 2.0f + (pulseAlpha * 3.0f), IM_COL32(160, 40, 210, (int)(100 * (1.0f - pulseAlpha))), 0, 4.0f);
            dl->AddCircle(center, buttonRadius + 4.0f + (pulseAlpha * 6.0f), IM_COL32(160, 40, 210, (int)(50 * (1.0f - pulseAlpha))), 0, 8.0f);
        }
        ...
        for (int i = 0; i < num_segments; i++) {
            ...
            dl->PathStroke(IM_COL32(160, 40, 210, 255), 0, 6.0f); 
        }

        // Main Solid Border (Slightly thicker as requested)
        dl->AddCircle(center, buttonRadius, IM_COL32(160, 40, 210, 255), 0, 5.5f);
        
        // Inner Fine Ring
        dl->AddCircle(center, buttonRadius - 5.0f, IM_COL32(190, 90, 230, 150), 0, 2.0f);
...
INLINE void DrawUpdateRequired(ImGuiIO& io) {
    ...
    // Draw Purple Border (Stylish bold look)
    dl->AddRect(winPos, winPos + ImVec2(cardW, cardH), IM_COL32(160, 40, 210, 200), 30.0f, 0, 13.0f);
    dl->AddRect(winPos + ImVec2(2, 2), winPos + ImVec2(cardW - 2, cardH - 2), IM_COL32(160, 40, 210, 40), 30.0f, 0, 1.0f);

    // Title Section: "UPDATE REQUIRED" (Purple bold text - moved higher)
    if (g_ArialBlackFont) PushFont(g_ArialBlackFont);
    ...
    TextColored(ImVec4(0.63f, 0.16f, 0.82f, 1.0f), O("UPDATE REQUIRED"));
    SetWindowFontScale(1.0f);
    if (g_ArialBlackFont) PopFont();
    ...
    // Line 3: Very small purple "Enjoy AKRO GOXI" below Line 2
    SetCursorPosY(cardH * 0.21f + 65.0f);
    SetWindowFontScale(0.80f); // Very small
    SetCursorPosX((cardW - totalW3) * 0.5f);
    TextColored(ImVec4(0.63f, 0.16f, 0.82f, 1.0f), "%s", enjoyText.c_str()); // Purple
    ...
    SetCursorPosX(70);
    PushStyleColor(ImGuiCol_Button, ImVec4(0.63f, 0.16f, 0.82f, 1.0f));
    PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.75f, 0.31f, 0.90f, 1.0f));
    PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.50f, 0.10f, 0.65f, 1.0f));
    ...
    DrawTeleLogo(cardW * 0.35f, O("https://t.me/akrogoxi")); // Left Icon
    DrawTeleLogo(cardW * 0.65f, O("https://t.me/akrogoxi")); // Right Icon
...
INLINE void DrawLogin(ImGuiIO& io) {
    ...
    if (is_logging_in) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            dl->AddCircleFilled(center + ImVec2(cosf(a), sinf(a)) * 50.0f, 9.0f, IM_COL32(160, 40, 210, (int)(255 * (8 - i) / 8.0f)));
        }
        ...
        dl->AddRect(inputPos, inputPos + inputSize, IM_COL32(160, 40, 210, 40), 20.0f, 0, 2.0f);
        ...
        // Clipboard Icon: FontAwesome Vector Icon (Purple)
        ...
        if (g_IconFont) {
            ...
            dl->AddText(iconDrawPos, IM_COL32(160, 40, 210, 255), "\uF46D");
            SetWindowFontScale(1.0f);
            PopFont();
        }
        ...
        if (Button(O("##AuthBtn"), ImVec2(cardW - 140, cardH * 0.17f))) {
            ...
            if (keyToUse == O("LION-56BF-8628-FFNK-9A2D")) {
                ...
                // Accepts two formats:
                //  1) Dashed: XXXXX-XXXXX-XXXXX-XXXXX-XXXXX (23 chars)
                //  2) Plain alphanumeric key with no dashes
                ...
                if (!isFormatValid && !keyToUse.empty()) {
                    bool allAlnum = true;
                    for (unsigned char c : keyToUse) {
                        if (!isalnum(c)) { allAlnum = false; break; }
                    }
                    isFormatValid = allAlnum;
                }
